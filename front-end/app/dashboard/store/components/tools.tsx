    ...
    <div className='bg-white p-2  flex flex-col gap-1'>
      {
        tools.map((tool,i)=>(
          toolSelected===i ?
          <button key={i} onClick={()=>dispatch(setTool(i))} className='btn bg-primary hover:bg-primary-focus text-white shadow-[0_5px_10px_2px_#570df815]  border-none '>{tool.icon}</button>
          :
          <button key={i} onClick={()=>dispatch(setTool(i))} className='btn bg-transparent text-gray-500 border-none '>{tool.icon}</button>
        ))
      }
    </div>
  )
